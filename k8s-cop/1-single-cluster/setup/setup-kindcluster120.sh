...
kind create cluster --config ../kindconfig/kindconfig-v120.yaml
sleep 1
kubectl apply -f https://raw.githubusercontent.com/metallb/metallb/v0.13.9/config/manifests/metallb-native.yaml --context kind-120
pods_checking=false
...
while ! $pods_checking; do
    ...
    if kubectl get pods -n metallb-system | grep -i controller | grep -q 'Running' && kubectl get pods -n metallb-system | grep -i speaker | grep -q 'Running' ; then
        ...
        kubectl create -f ../kindconfig/metallb-v0139-ipaddress-pool-1.yaml --context kind-120
...
sleep 1
kubectl config rename-context kind-1210 120
watch kubectl get pods -A --context 120